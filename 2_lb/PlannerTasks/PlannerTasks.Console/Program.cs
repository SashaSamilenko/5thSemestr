...
namespace PlannerTasks.Console
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //efUnitOfWork.Save();
            using (TasksContext db = new TasksContext("PlannerDB"))
            {
                var taskRepository = new TaskRepository(db);
                taskRepository.Create(new Task
                {
                    Description = "asdasd", Priority = Priority.High, Status = Status.NotStarted,
                    TimeExecution = new TimeSpan(0, 3, 0, 0), StartTime = DateTime.Now, EmployeeId = 1
                });
